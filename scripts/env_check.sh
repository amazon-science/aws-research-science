...
instructions="\n\n🔬 DS-EXP PLUGIN: Automatic Experiment Tracking\n\nWhen user requests training/ML tasks, automatically track and queue experiments:\n\n**PREFERRED: Use Job Queue (auto-launches when GPU free)**\n./ds-exp-plugin/scripts/queue_experiment.sh \"exp_name\" \"python train.py\" [gpu_mem_mb]\n\nQueue system:\n- Checks GPU availability\n- Launches immediately if GPU free\n- Queues and auto-launches later if GPUs busy\n- Retries failed launches (up to 3x)\n- Start watcher: ./ds-exp-plugin/scripts/queue_start_watcher.sh\n- Check status: ./ds-exp-plugin/scripts/queue_status.sh\n\n**Manual tracking (if not using queue):**\n1. Start: EXP_FILE=\$(./ds-exp-plugin/scripts/start_experiment.sh \"name\" \"desc\" [gpu])\n2. In training code:\n   import subprocess\n   def report(name, val): subprocess.run(['./ds-exp-plugin/scripts/report_metric.sh', name, str(val)])\n   def note(text): subprocess.run(['./ds-exp-plugin/scripts/report_note.sh', text])\n3. Complete: ./ds-exp-plugin/scripts/complete_experiment.sh completed \$EXP_FILE\n\nView: /ds-exp:dash"
